...
This document details the process of fine-tuning the [`Qwen/Qwen3-4B-Instruct-2507`](https://huggingface.co/Qwen/Qwen3-4B-Instruct-2507) model to generate high-quality satirical news headlines in the style of *The Onion*. The final model demonstrate a signficant improvement over the baseline, producing satire that is often clever, nuanced, and context-aware.

## The Baseline's Limitations

The base `Qwen3-4B-Instruct-2507` model, while a powerful instruction-follower, struggles with the generating satirical headlines. When prompted, its outputs are forumlaic, defaulting to standard tropes and relying heavily on pure absurdity.
...
The first major goal was to setup a baseline finetuning pipeline. While that may seem like an easy goal, subtle errors in implementation details or hyperparameter selection can lead to significant changes in model performance. I turned to HuggingFace's excellent LLM ecosystem:
...
- The result of my first run had multiple special tokens, such as `<|think|>` and `<|tool_call|>` littered in the generated text. I realized that while the model is not intended to generate `<think></think>` blocks, the chat template was inserting them. I needed to create my own corrected chat template.
- In my second run, I attempted to fix the chat template and to increase the quantization of the model weights to `NF4`. The results were much less coherent. It seems like smaller models are harder to quantize. A third run at 8bit quantization resolved the issue.
...
While the headlines are much more varied, the often sound like standard news headlines, with minimal satirical wit or social commentary. In other words, it copied the style but not the substance.

### Phase 2: Optimization (Model T02)

The baseline Model T01 was quite slow. The first several run took 3 hours each. This significant increased the friction in iterating, and development much more frustating. In one situation, a small typo in the chat template (`<im_end>`) invalidated a whole run.

My focus in this stage was optimizing the runtime. Three were three significant jumps in performance.
1. Setting the batch size to `8` from the initial, conservative `2` reduced the runtime to 1 hour - 1 hour 30 minutes. Training stability was not effected because we accumulated gradients over multiple batches.
...
However, that lead me to experiment with enabling thinking mode / chain of thought. I researched methods for writing satirical headlines and read multiple articles. At its core, there are 3 primary steps.

1. Identify the **subject** of satire: Who or what are we poking fun at?
2. Brainstorm a **satirical angle**: What rhetorical device can we apply to the subject (e.g. exaggeration, irony, understandment, etc.)?
3. Compose the **headline**: How can we distill the subject and satirical angle into a single snappy title.

While prompting the baseline `Qwen3-8B` with that chain of thought resulted in a few jems, it was often prone to run-on generation and there were significant issues with the model's thought process.

**Change of Though Finetune Examples**
```
...
While not perfect we can a significant improvement in substance.
